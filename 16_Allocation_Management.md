...
| --- | --- | --- | --- | --- | --- |
| Pools 頁籤 | 點擊 | 查看已生效配置 | 查詢 Allocation、Pool、Project、System | 否 | 顯示 Shared Allocation 與 Dedicated Assignment |
| Requests 頁籤 | 點擊 | 查看待處理申請 | 查詢 Allocation Request | 否 | 顯示 Pending Request 清單 |
| KPI 區 | 顯示 | 提供資源配置概況 | 彙總 HCM 本地資料 | 否 | 顯示配置數、待處理數與風險數 |
...
| UI 元件 | Event | 業務動作 | 異動/查詢資料 | 是否需要 Provider | 畫面結果 |
| --- | --- | --- | --- | --- | --- |
| Add Allocation | 點擊 | 為 Shared Pool 新增正式配額 | 查詢 Pool、Project、System、Subnet | 視 Provider 而定 | 開啟 Shared Allocation 表單 |
| Edit | 點擊 | 修改既有 Shared Allocation | 查詢既有 Allocation | 視 Provider 而定 | 開啟 Shared Allocation 表單 |
| Remove | 點擊 | 解除 Shared Allocation | 刪除或停用 Allocation | 視 Provider 而定 | Allocation 從表格移除 |
| 新增指派 | 點擊 | 手動建立 Dedicated Pool 指派 | 查詢可用 Dedicated Pool、Project、System | 否，除非 Provider 有附屬資源 | 開啟 Dedicated 指派表單 |
| 解除指派 | 點擊 | 解除 Dedicated Pool 與系統的關係 | 刪除或停用 Allocation | 視 Provider 而定 | Dedicated Assignment 從清單移除 |
...
| UI 元件 | Event | 業務動作 | 異動/查詢資料 | 是否需要 Provider | 畫面結果 |
| --- | --- | --- | --- | --- | --- |
| Project | 選擇 | 指定配置歸屬專案 | 查詢 HCM Project 並更新表單 | 否 | 更新可選系統 |
| System | 選擇 | 指定配置歸屬系統 | 查詢 HCM System 並更新表單 | 否 | 顯示選定系統 |
| CPU | 輸入 | 設定 CPU 配額 | 更新表單資料 | 否 | 顯示配額數值 |
| Memory | 輸入 | 設定記憶體配額 | 更新表單資料 | 否 | 顯示配額數值 |
| Disk | 輸入 | 設定磁碟配額 | 更新表單資料 | 否 | 顯示配額數值 |
| Subnet checkbox | 勾選/取消 | 指定此 Allocation 可用網段 | 更新表單資料 | 否 | 更新已選 Subnet |
| Provider Extension 欄位 | 輸入/選擇 | 補充 Provider 特有配置資訊 | 更新表單資料 | 視 Provider 而定 | 顯示 Provider 差異資料 |
| Cancel | 點擊 | 放棄本次設定 | 不異動資料 | 否 | 關閉表單 |
| Save | 點擊 | 建立或更新 Shared Allocation | 新增或更新 HCM Allocation | 視 Provider 而定 | 表格更新，必要時更新 Request mount 狀態 |
...
| UI 元件 | Event | 業務動作 | 異動/查詢資料 | 是否需要 Provider | 畫面結果 |
| --- | --- | --- | --- | --- | --- |
| Dedicated Pool | 選擇 | 指定要指派的專用資源池 | 查詢 HCM Pool | 否，除非 Provider 有附屬資源 | 更新表單 |
| Project | 選擇 | 指定指派歸屬專案 | 查詢 HCM Project | 否 | 更新可選系統 |
| System | 選擇 | 指定指派歸屬系統 | 查詢 HCM System | 否 | 顯示選定系統 |
| 取消 | 點擊 | 放棄本次指派 | 不異動資料 | 否 | 關閉表單 |
| 建立指派 | 點擊 | 建立 Dedicated Allocation | 新增 HCM Allocation | 視 Provider 而定 | Dedicated Assignment 清單更新 |
...
| UI 元件 | Event | 業務動作 | 異動/查詢資料 | 是否需要 Provider | 畫面結果 |
| --- | --- | --- | --- | --- | --- |
| Configure | 點擊 | 針對 Shared Pool 申請建立正式 Allocation | 查詢 Request、Pool、Project、System、Subnet | 視 Provider 而定 | 開啟 Shared Allocation 表單並帶入申請資料 |
| Execute | 點擊 | 完成整張 Request | 更新 Request 狀態，建立必要 Dedicated Allocation | 視 Provider 而定 | Request 從待處理清單移除 |
| Request Card | 顯示 | 讓管理者檢查申請內容 | 查詢 HCM Allocation Request | 否 | 顯示專案、系統、Pool 與完成狀態 |
...
| UI 元件 | Event | 業務動作 | 異動/查詢資料 | 是否需要 Provider | 畫面結果 |
| --- | --- | --- | --- | --- | --- |
| Cancel | 點擊 | 取消解除 | 不異動資料 | 否 | 關閉確認視窗 |
| Confirm | 點擊 | 解除已生效 Allocation | 刪除或停用 HCM Allocation | 視 Provider 而定 | Allocation 清單更新 |
