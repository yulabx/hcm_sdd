...
|---|---|---|---|---|---|
| 新增 Cloud Provider | 點擊 | 開啟 provider 設定表單 | 無資料異動，等待使用者輸入 | 否，僅使用 HCM 本地資料 | 顯示 Provider 表單 |
| 編輯 Provider | 點擊 | 開啟既有 provider 設定 | 查詢 provider 目前設定 | 否，僅使用 HCM 本地資料 | 顯示 Provider 表單 |
| 刪除 Provider | 點擊並確認 | 移除 provider 定義 | Provider 資料；需注意既有 connection 數 | 否，僅使用 HCM 本地資料 | Provider 區塊移除 |
| 新增 Connection | 點擊 | 開啟 connection 表單，預設 provider | 無資料異動，等待使用者輸入 | 否，僅使用 HCM 本地資料 | 顯示 Connection 表單 |
| 編輯 Connection | 點擊 | 開啟既有 connection 設定 | 查詢 connection 目前設定；敏感資訊以已設定標記呈現 | 否，僅使用 HCM 本地資料 | 顯示 Connection 表單 |
| 刪除 Connection | 點擊並確認 | 移除 connection | Connection 資料 | 否，僅使用 HCM 本地資料 | Connection Row 移除 |
| 同步資料 | 點擊 | 開啟同步資料 Wizard | 查詢 connection、pool、subnet、security group、VM 資料 | 否，開啟畫面本身不進 provider | 顯示同步 Wizard |
...
| UI 元件 | Event | 業務動作 | 異動/查詢資料 | 是否需要 Provider | 畫面結果 |
|---|---|---|---|---|---|
| Provider 表單儲存 | 點擊儲存 | 新增或更新 provider 定義 | Provider 資料 | 否，僅使用 HCM 本地資料 | Provider 區塊更新 |
| Provider 表單取消 | 點擊取消/關閉 | 放棄本次編輯 | 無資料異動 | 否，僅使用 HCM 本地資料 | 回到主畫面 |
...
| UI 元件 | Event | 業務動作 | 異動/查詢資料 | 是否需要 Provider | 畫面結果 |
|---|---|---|---|---|---|
| Connection 表單儲存 | 點擊儲存 | 建立或更新 provider 連線設定與授權摘要 | Connection 資料；敏感資訊需遮罩 | 否；後續同步或外部授權才會進 provider | Connection Row 更新 |
| Connection 表單下一步 | 點擊儲存並下一步，若 provider 需要 | 儲存 provider-specific 授權所需的基本資料並進入授權差異區 | Connection 資料 | 否，尚未啟動外部授權 | 顯示 Provider 授權差異區 |
| Connection 表單取消 | 點擊取消/關閉 | 放棄本次編輯 | 無資料異動 | 否，僅使用 HCM 本地資料 | 回到主畫面 |
...
| UI 元件 | Event | 業務動作 | 異動/查詢資料 | 是否需要 Provider | 畫面結果 |
|---|---|---|---|---|---|
| Provider-specific 授權動作 | 點擊 | 依 provider plugin 定義執行授權步驟 | Connection 授權狀態 | 是，使用「Provider 授權」plugin 能力 | 顯示 provider 定義的授權結果或下一步提示 |
| 檢查授權結果 | 點擊，若 provider 需要 | 查詢 provider 授權是否完成 | Connection 授權狀態與授權摘要 | 是，使用「Provider 授權」plugin 能力 | 顯示授權完成、等待中或失敗 |
| 完成 | 點擊 | 結束授權視窗 | 無資料異動 | 否，僅使用 HCM 本地資料 | 回到主畫面 |
...
| UI 元件 | Event | 業務動作 | 異動/查詢資料 | 是否需要 Provider | 畫面結果 |
|---|---|---|---|---|---|
| 同步 Pool | 點擊 | 從 provider 取得資源池與容量 | 更新 Pool，保留 provider 來源識別 | 是，使用「同步資源池」plugin 能力 | Pool 清單更新，未補欄位標示待補 |
| Pool 欄位編輯 | 修改欄位 | 補齊 HCM 使用 Pool 所需業務欄位 | Pool 草稿 | 否，僅使用 HCM 本地資料 | 欄位即時更新 |
| Pool 設定下一步 | 點擊下一步 | 檢查並儲存 Pool 的 HCM 業務欄位 | Pool 的名稱、Site、Region、Allowed Env、容量校正 | 否，僅使用 HCM 本地資料 | 進入 Network step |
...
| UI 元件 | Event | 業務動作 | 異動/查詢資料 | 是否需要 Provider | 畫面結果 |
|---|---|---|---|---|---|
| 同步 Network | 點擊 | 從 provider 取得網路與安全群組 | 更新 Subnet、Security Group | 是，使用「同步網路」plugin 能力 | Subnet / Security Group 清單更新 |
| Subnet Type 編輯 | 選擇類型 | 補齊 HCM 對 subnet 的用途分類 | Subnet 草稿 | 否，僅使用 HCM 本地資料 | Subnet 待補狀態更新 |
| Network 設定下一步 | 點擊下一步 | 檢查並儲存 Subnet Type | Subnet 類型 | 否，僅使用 HCM 本地資料 | 進入 Template/Image step |
...
| UI 元件 | Event | 業務動作 | 異動/查詢資料 | 是否需要 Provider | 畫面結果 |
|---|---|---|---|---|---|
| 同步 Template/Image | 點擊 | 從 provider 取得建立 VM 的規格來源 | 更新 Pool VM Catalog | 是，使用「同步 VM 規格來源」plugin 能力 | Template 或 Image 清單更新 |
| 切換 Pool catalog 檢視 | 展開或瀏覽清單 | 查看某 pool 的 VM 建立選項 | 查詢已同步 VM Catalog | 否，僅使用 HCM 本地資料 | 顯示對應 Pool 的 catalog |
...
| UI 元件 | Event | 業務動作 | 異動/查詢資料 | 是否需要 Provider | 畫面結果 |
|---|---|---|---|---|---|
| 同步 VM | 點擊 | 從 provider 取得既有 VM 清單與狀態 | 更新 VM 資料與 provider 來源識別 | 是，使用「同步 VM 清單」plugin 能力 | VM 數量更新，初始化完成 |
| 關閉 Wizard | 點擊 x | 離開同步流程 | 無資料異動 | 否，僅使用 HCM 本地資料 | 回到 Cloud Settings 主畫面 |
