...
| 欄位 | 業務意義 | 必填 | 範例 | 備註 |
|---|---|---|---|---|
| provider_pool_id | provider 原始資源池識別 | 是 | vdc-001、cluster-01 | 後續同步與 VM 操作用 |
| name | 資源池名稱 | 是 | prod-vdc | 可作為 HCM 預設顯示名稱 |
| site | 站點或區域 | 否 | primary、ap-northeast-1 | 沒有時可由管理員補 |
| capacity_cpu | CPU 總量 | 否 | 64 | HCM 標準單位 |
| capacity_memory_gb | Memory 總量 | 否 | 512 | GB |
| capacity_disk_tb | Disk 總量 | 否 | 20 | TB |
| cpu_provisioned | provider 回報 CPU 已配置量 | 否 | 32 | 指標意義需在 provider 文件說明 |
| mem_provisioned_gb | provider 回報 Memory 已配置量 | 否 | 256 | GB |
| disk_provisioned_tb | provider 回報 Disk 已配置量 | 否 | 8 | TB |
...
|---|---|---|---|---|
| Harvester | `provider_pool_id`、`name` | `capacity_cpu`、`capacity_memory_gb`、`capacity_disk_tb` | `cpu_provisioned`、`mem_provisioned_gb`、`disk_provisioned_tb` | 來源通常為 Node + Longhorn；`site` 可由連線設定或管理者補 |
| VMware Cloud Director | `provider_pool_id`、`name` | `capacity_cpu`、`capacity_memory_gb`、`capacity_disk_tb` | `cpu_provisioned` 或 `used_cpu`（擇一） | CPU 來源常為 MHz；Memory/Storage 需換算至 GB/TB |
| AWS | `provider_pool_id`、`name` | 可留空 | 可留空 | VPC 不直接提供 pool 級 CPU/Memory/Disk 容量，建議以 `raw_summary` 補充 |
| vSphere | `provider_pool_id`、`name` | `capacity_cpu`、`capacity_memory_gb`、`capacity_disk_tb` | `used_cpu` 或 `cpu_provisioned`、`used_memory_gb` 或 `mem_provisioned_gb`（各擇一） | 來源常見 `total_cpu_mhz`/`used_cpu_mhz`、`*_memory_gib`；需換算成標準欄位 |
...
| 欄位 | 業務意義 | 必填 | 範例 | 備註 |
|---|---|---|---|---|
| templates | template 型 VM 建立來源 | 否 | Ubuntu Template | VCD、vSphere 常見 |
| images | image 型 VM 建立來源 | 否 | AMI、Harvester Image | AWS、Harvester 常見 |
| flavors | 固定規格選項 | 否 | t3.medium | AWS 常見 |
...
| 欄位 | 業務意義 | 必填 | 範例 | 備註 |
|---|---|---|---|---|
| provider_vm_id | provider 建立出的 VM 識別 | 是 | i-001 | 後續追蹤使用 |
...
| 欄位 | 業務意義 | 必填 | 範例 | 備註 |
|---|---|---|---|---|
| provider_vm_id | provider VM 識別 | 是 | i-001 | 開關機目標 |
