...
- VM 建立若超出 Pool / Allocation 配額，是否允許送出由 Cloud Provider 的 `allow_quota_overcommit` 決定；AWS / VCD 預設 `false`，vSphere / Harvester 預設 `true`。
...
| Pool 容量摘要 | 依資源視角顯示已申請或使用量 | HCM Pool |
| 資源視角 | 切換 Pool bar 分子語意 | 前端全域狀態，預設已申請 |
...
| Resource View Toggle | 點擊 | 切換 Pool list 與 Pool detail 的容量圖表 | 讀取 HCM Pool `*_provisioned_*` 或 `*_used_*` 欄位 | 無額外 API | Pool bar 即時更新 |
...
| CPU / Memory / Disk 容量摘要 | 依資源視角呈現已申請或使用量佔總量 | HCM Pool |
...
| UI 元件 | Event | 業務動作 | 異動/查詢資料 | HCM Backend API | 畫面結果 |
| --- | --- | --- | --- | --- | --- |
| Resource Bar | 顯示 | 依資源視角呈現 Pool 容量狀態 | 讀取 HCM Pool `*_total_*`、`*_provisioned_*`、`*_used_*` | `GET /api/pools` | 顯示已申請或使用量與總量 |
...
| Cloud Provider 設定 | 前端行為 | 後端行為 |
|---|---|---|
| `allow_quota_overcommit=false` | 顯示資源不足並停用建立按鈕 | `POST /api/vms` 回傳錯誤，不建立 VM |
| `allow_quota_overcommit=true` | 顯示超配警示但允許送出 | 允許建立，仍保留資源檢查結果供 UI 顯示 |
