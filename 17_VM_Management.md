...
| --- | --- | --- | --- | --- | --- |
| 進入頁面 | 載入 | 查詢可見 Pool、VM、Allocation | 查詢 HCM Pool、VM、Allocation | 否 | 顯示 Pool 清單與預設 Pool Detail |
| Pool Card | 點擊 | 切換目前管理的 Pool | 讀取選定 Pool、VM、Allocation | 否 | 右側顯示該 Pool 的 VM 資訊 |
| VM 狀態自動更新 | Provider 回寫或同步 | 更新 VM 狀態、IP、NIC 等資訊 | 更新 HCM VM | 視 Provider 而定 | VM 狀態與清單即時更新 |
...
| UI 元件 | Event | 業務動作 | 異動/查詢資料 | 是否需要 Provider | 畫面結果 |
| --- | --- | --- | --- | --- | --- |
| Search | 輸入 | 依關鍵字篩選 Pool | 查詢或過濾 HCM Pool | 否 | Pool 清單縮小 |
| Type Filter | 點擊 | 篩選 Dedicated / Shared | 讀取 Pool type | 否 | Pool 清單更新 |
| Cloud Filter | 選擇 | 篩選指定 Cloud | 讀取 HCM Cloud / Pool | 否 | Pool 清單更新 |
| Project Filter | 選擇 | 篩選指定 Project 可使用的 Pool | 讀取 Allocation 與 Project | 否 | Pool 清單更新 |
| System Filter | 選擇 | 篩選指定 System 可使用的 Pool | 讀取 Allocation 與 System | 否 | Pool 清單更新 |
...
| UI 元件 | Event | 業務動作 | 異動/查詢資料 | 是否需要 Provider | 畫面結果 |
| --- | --- | --- | --- | --- | --- |
| Resource Bar | 顯示 | 呈現 Pool 或 Allocation 使用狀態 | 彙總 HCM Pool、Allocation、VM | 否 | 顯示使用量與剩餘量 |
| Add VM | 點擊 | 開始在目前 Pool 建立 VM | 查詢 Pool、Allocation、Catalog、Subnet | 建立送出時才需要 | 開啟 VM 建立表單 |
...
| UI 元件 | Event | 業務動作 | 異動/查詢資料 | 是否需要 Provider | 畫面結果 |
| --- | --- | --- | --- | --- | --- |
| Add VM | 點擊 | 在 Dedicated Pool 建立 VM | 查詢 Pool、Catalog、Subnet | 建立送出時才需要 | 開啟 VM 建立表單 |
| VM Row | 點擊或 View | 檢視 VM 詳細資料 | 查詢 HCM VM | 否 | 開啟 VM 檢視表單 |
| Edit | 點擊 | 修改 VM 在 HCM 的基本資料 | 查詢 HCM VM | 否 | 開啟 VM 編輯表單 |
| Delete | 點擊 | 準備刪除 VM 資料 | 查詢 HCM VM | 否 | 開啟刪除確認 |
| Start / Stop | 點擊 | 準備變更 VM 電源狀態 | 查詢 HCM VM | 確認後需要 | 開啟操作確認 |
...
| UI 元件 | Event | 業務動作 | 異動/查詢資料 | 是否需要 Provider | 畫面結果 |
| --- | --- | --- | --- | --- | --- |
| Allocation Section | 顯示 | 依 Allocation 分組顯示 VM | 查詢 HCM Allocation、VM | 否 | 顯示該 Allocation 的 VM 與剩餘額度 |
| Add VM | 點擊 | 在指定 Allocation 下建立 VM | 查詢 Allocation、Catalog、Subnet | 建立送出時才需要 | 開啟 VM 建立表單並帶入 Allocation |
| Unassigned VM | 顯示 | 呈現同步回來但尚未歸屬 Allocation 的 VM | 查詢 HCM VM | 否 | 顯示為待整理資料 |
| VM Actions | 點擊 | 檢視、修改、刪除或開關機 | 查詢 HCM VM | 視操作而定 | 開啟對應表單或確認 |
...
| UI 元件 | Event | 業務動作 | 異動/查詢資料 | 是否需要 Provider | 畫面結果 |
| --- | --- | --- | --- | --- | --- |
| Allocation | 選擇 | 指定 Shared Pool VM 使用哪份額度 | 查詢 HCM Allocation | 否 | 更新可用配額與可選 Subnet |
| VM Name | 輸入 | 設定 VM 顯示名稱與建立名稱 | 更新表單資料 | 否 | 顯示名稱 |
| Hostname | 輸入 | 設定 VM 主機名稱 | 更新表單資料 | 建立送出時依 provider | 顯示 hostname |
| Template / Image / Flavor | 選擇 | 指定 VM 建立來源 | 查詢 HCM VM Catalog | 建立送出時依 provider | 帶入預設 CPU / Memory / Disk 或規格摘要 |
| CPU / Memory / Disk | 輸入 | 設定 VM 規格與額度消耗 | 更新表單資料並檢查配額 | 否 | 顯示剩餘額度與是否超額 |
| Add / Remove Disk | 點擊 | 調整 VM 磁碟需求 | 更新表單資料並檢查配額 | 建立送出時依 provider | 磁碟清單與總容量更新 |
| Subnet | 選擇 | 指定 VM 使用網段 | 查詢 HCM Subnet | 建立送出時依 provider | 更新 IP 與網路資訊 |
| IP Mode | 選擇 | 指定自動或固定 IP | 更新表單資料 | 建立送出時依 provider | 顯示或停用 IP 欄位 |
| Security Group | 選擇/顯示 | 指定或呈現 VM 安全群組 | 查詢 HCM Security Group | 建立送出時依 provider | 安全群組資料更新 |
| Initial Status | 選擇 | 指定建立後期望狀態 | 更新表單資料 | 建立送出時依 provider | 顯示期望狀態 |
| Cancel | 點擊 | 放棄本次建立或修改 | 不異動資料 | 否 | 關閉表單 |
| Save | 點擊 | 建立 VM 或更新 VM 基本資料 | 新增或更新 HCM VM | 建立 VM 時視 provider 而定 | 成功後 VM 清單更新 |
...
| Provider VM 識別 | Provider 操作目標 | HCM VM cloud_ref |
| 目標狀態 | 追蹤操作完成後狀態 | 操作類型 |
...
| UI 元件 | Event | 業務動作 | 異動/查詢資料 | 是否需要 Provider | 畫面結果 |
| --- | --- | --- | --- | --- | --- |
| Start | 點擊 | 準備啟動 stopped VM | 查詢 HCM VM | 確認後需要 | 開啟確認視窗 |
| Stop | 點擊 | 準備停止 running VM | 查詢 HCM VM | 確認後需要 | 開啟確認視窗 |
| Cancel | 點擊 | 放棄本次電源操作 | 不異動資料 | 否 | 關閉確認視窗 |
| Confirm Start | 點擊 | 呼叫 provider 啟動 VM | 更新 HCM VM 狀態為 starting，後續追蹤 running | 是，VM 開機掛載點 | VM 狀態顯示開機中，完成後更新 |
| Confirm Stop | 點擊 | 呼叫 provider 停止 VM | 更新 HCM VM 狀態為 stopping，後續追蹤 stopped | 是，VM 關機掛載點 | VM 狀態顯示關機中，完成後更新 |
...
| --- | --- | --- | --- | --- | --- |
| Delete | 點擊 | 準備刪除 VM 資料 | 查詢 HCM VM | 否 | 開啟刪除確認 |
| Cancel | 點擊 | 放棄刪除 | 不異動資料 | 否 | 關閉確認視窗 |
| Confirm Delete | 點擊 | 刪除 HCM VM 資料 | 刪除或停用 HCM VM | 目前不在本文件定義 provider 刪除 | VM 從清單移除 |
