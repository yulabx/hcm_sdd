...
| Sync VM Inventory Input | Cloud Connection | 同步 VM 清單 | 同步全叢集所有 Namespace 的 VM |
...
| --- | --- |
| 授權型態 | 表單式授權 |
| Connection 表單欄位 | Base URL、TLS Skip Verify、API Token |
| 是否顯示授權差異區 | 否 |
| 授權差異區欄位 | 無 |
...
| --- | --- | --- | --- | --- | --- |
| Cloud Settings | Connection 表單 | 顯示 Cluster 設定語意 | Base URL、TLS Skip Verify、API Token | 可建立/更新連線 | Token 必須遮罩；自簽憑證可跳過 TLS |
...
| --- | --- | --- |
| `provider_pool_id` | `metadata.id` | 必填；Harvester 以整個 Cluster 作為一個 Pool；預設使用 metadata.id |
